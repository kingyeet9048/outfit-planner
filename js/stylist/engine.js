...
function bucket(items) {
  return {
    ...
    pant: items.filter(i => i.category === 'pant'),
...
function generateOne(buckets, intent, used, rng) {
  if (!buckets.top.length || !buckets.shoes.length) return null;

  // Anchor on the top — it's the most expressive piece.
  const top = pickFirstByFormality(buckets.top, intent, used, rng);
  if (!top) return null;
  used.add(top.id);

  const pant = pickBest(buckets.pant, top, intent, used, rng);
  if (pant) used.add(pant.id);

  const shoesAnchor = pant || top;
  ...
  for (let i = 0; i < numAcc; i++) {
    const a = pickBest(buckets.accessory, top, intent, used, rng);
    if (!a) break;
    used.add(a.id);
    accs.push(a);
  ...
  if (intent.weather === 'rainy' || intent.weather === 'cold') {
    const o = pickBest(buckets.other, top, intent, used, rng);
    if (o) { used.add(o.id); others.push(o); otherIds.push(o.id); }
  }

  const slots = [top, pant, shoe, ...accs, ...others].filter(Boolean);
  ...
  return {
    ...
    accessoryIds,
    otherIds,
    _meta: { top, pant, shoe, accs, others, harmonyLabel, intent }
  };
}
...
export function refineOutfit(itemContext, intent, baseOutfit, options = {}) {
  ...
  const anchorId = newOutfit.topId || newOutfit.pantId;
  ...
  if (swap.swapTop && anchor) {
    ...
    const t = pickBest(buckets.top, anchor, intent, used, rng);
    if (t) { newOutfit.topId = t.id; used.add(t.id); }
  ...
  // Recompute meta
  const top = itemContext.find(i => i.id === newOutfit.topId);
  ...
  const others = (newOutfit.otherIds || []).map(id => itemContext.find(i => i.id === id)).filter(Boolean);
